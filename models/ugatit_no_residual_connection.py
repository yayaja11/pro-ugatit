...
class Generator(nn.Module):
    def __init__(self, input_nc, output_nc,
        ...
        #Ldownsampling_s4 = nn.Sequential(*downsampling_s4)
        ...
        '''decoder_s2 = [nn.UpsamplingBilinear2d(scale_factor=2),
                      nn.ReflectionPad2d(1),                     
                      nn.Conv2d(ngf*mult, ngf*mult, 3, 1, 0),
                      ResBlockByAdaLIN(ngf*mult, ngf*mult,anime=anime),
                      ResBlockByAdaLIN(ngf*mult, ngf*mult,anime=anime)]'''
        ...
        '''decoder = [nn.UpsamplingBilinear2d(scale_factor=2),
                      nn.ReflectionPad2d(1),                     
                      nn.Conv2d(ngf*mult, ngf*mult, 3, 1, 0),
                      ResBlockByAdaLIN(ngf*mult, ngf*mult,anime=anime),
                      ResBlockByAdaLIN(ngf*mult, ngf*mult,anime=anime)]'''
        ...
        '''decoder2x.append(nn.UpsamplingBilinear2d(scale_factor=2))
        decoder2x.append(nn.ReflectionPad2d(1))
        decoder2x.append(nn.Conv2d(ngf*mult, ngf*mult//2, 3, 1, 0))
        #decoder2x.append(LayerInstanceNorm(ngf*mult//2))

        decoder2x.append(ResBlockByAdaLIN(ngf*mult//2, out_dim=ngf*mult//2, anime=anime))
        decoder2x.append(ResBlockByAdaLIN(ngf*mult//2, out_dim=ngf*mult//2 , anime=anime))'''
        
        '''decoder4x.append(nn.UpsamplingBilinear2d(scale_factor=2))
        decoder4x.append(nn.ReflectionPad2d(1))
        decoder4x.append(nn.Conv2d(ngf*mult//2, ngf*mult//4, 3, 1, 0))
        #decoder4x.append(LayerInstanceNorm(ngf*mult//4))

        decoder4x.append(ResBlockByAdaLIN(ngf*mult//4,out_dim=ngf*mult//4 ,anime=anime))
        decoder4x.append(ResBlockByAdaLIN(ngf*mult//4,out_dim=ngf*mult//4, anime=anime))    '''



        #self.adain_resblocks = nn.Sequential(*adain_resblock)
        ...
        '''self.decoder_s4 = nn.Sequential(*decoder_s4)
        self.decoder_s2 = nn.Sequential(*decoder_s2)
        self.decoder = nn.Sequential(*decoder)
        self.decoder2x = nn.Sequential(*decoder2x)
        self.decoder4x = nn.Sequential(*decoder4x)'''
    ...
    def forward(self, img, step=0,alpha=0):
        ...
        b, c, h, w = x.shape
        gamma, beta = self.mlp(self.gap(x).view(b, c))  
